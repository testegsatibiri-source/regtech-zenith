...
- **DEBT-022 · PH Payroll Correctness — commercial readiness gap.** PH pack v1.1.0 (PH-2024.2) now implements Phase 1 fiscal corrections: SSS RA 11199 stepped table (P1a), 13th-month PD 851 earned base via `annualGrossEarned` (P1b), and BIR ₱90k tax exemption ceiling via `nonTaxableBenefits`/`cumulativeTaxableBenefits` (P1c). NCR regional minimum wage is parameterized (P1d). Phase 2 (statutory identifiers) closed in the same sprint: `engines/identifiers.ts` validates employee TIN/SSS/PhilHealth/Pag-IBIG and the employer registry (TIN+branch, RDO, SSS/PhilHealth/Pag-IBIG employer numbers, persisted in `companies.statutory_metadata`), scored by rule `PH-STAT-IDS` and heuristics `PH-STAT-IDS-COVERAGE` / `PH-STAT-IDS-FORMAT`. Phase 3 (staggered statutory deadlines) closed in Sprint H21: `engines/deadlines.ts` resolves SSS (ER last digit, Circular 2021-005), PhilHealth (PEN last digit, Circular 2020-0025), Pag-IBIG (employer name letter, HDMF Circular 274) and BIR 1601-C (eFPS group RR 26-2002), with weekend/regular-holiday roll-forward and `needs_review` flagging when the employer registry lacks the identifier. `CalendarProvider` gained the optional `CalendarSubject` argument (additive, interface v1 preserved); pack bumped to v1.2.0 / PH-2024.3 and re-signed. Phase 4 (filing exports) closed in Sprint H21: optional SDK capability `filings` + `FilingProvider`, PH generators for BIR 1601-C, BIR 1604-C Alphalist (DAT), SSS R-3, PhilHealth RF-1 and Pag-IBIG MCRF, stored in `statutory_filings` with SHA-256 checksum, `ruleset_version`/`pack_version` stamps, submission receipts and amended-filing path (ADR-0037). Pack re-signed at v1.3.0. **Status:** Phases 1–4 done; `commercialReady` stays `false` until the generated layouts are validated against a real portal upload with a pilot employer (Phase 5).
  ...
  **H24 (2026-09-16) — statutory evidence & table validity.** B3 closed: every PH statutory table carries structured provenance (`PH_PARAMS.statutorySources`, ids in `src/packs/philippines/constants.ts`) with `effectiveFrom` + `sourceStatus`; invariants enforced by `src/packs/philippines/__tests__/params-validity.test.ts` (presence, evidence linkage, effectiveFrom coherence, author≠reviewer sign-off). Wage regions re-shaped to an array (forward-compatible with B4) behind `phMinWageMonthlyFloor()`; golden test `wage-golden.test.ts` pins the pre-refactor NCR boundary (₱13,420). B2a closed: internal evidence files in `docs/governance/legal-opinions/PH-*-2026-09-16.md` (3 official: PhilHealth, BIR, Pag-IBIG; 2 stale: SSS → DEBT-030, NCR wage → DEBT-031). **Still open:** B2b (external legal opinion — Philippine-licensed labor/tax lawyer, IBP number, temporal validity scope) and B1 (pilot portal validation) keep `commercialReady: false`. Reconciliation found the SSS table superseded by Circular 2024-006 (eff. 2025-01-01) and the NCR wage value behind Wage Orders NCR-24/26/27 (current ₱755/day since 2026-07-25).

- **DEBT-030 · PH SSS table superseded (H24 finding, 2026-09-16).** `PH_PARAMS.sss` is the 2024 schedule (14%, MSC ₱4,000–₱30,000). SSS Circular 2024-006 (SSC Res. 560-s.2024), effective 2025-01-01, sets 15% (5% EE / 10% ER), MSC ₱5,000–₱35,000 + MPF tier. Correction requires a params/rulesetVersion bump, re-signature and golden re-baseline (out of H24 scope, which is zero-calculated-value-change). Evidence: `docs/governance/legal-opinions/PH-sss-msc-2026-09-16.md`. Owner: PH pack.
- **DEBT-031 · PH NCR minimum wage stale (H24 finding, 2026-09-16).** `PH_PARAMS.wageRegions[NCR]` carries ₱610/day (Wage Order NCR-23, eff. 2023-07-16); superseded by NCR-24 (₱645, 2024-07-17), NCR-26 (₱695, 2025-07-18) and NCR-27 (₱755 non-agri, 2026-07-25; NCR-28 published 2026-09-11 pending effectivity). Same correction path as DEBT-030. Evidence: `docs/governance/legal-opinions/PH-wage-ncr-2026-09-16.md`. Owner: PH pack.
  - **Closed 2026-09-23 (Etapa A, audit plan):** pack bumped to v1.7.0 / PH-2025.1. SSS now 15% (5% EE / 10% ER), MSC ₱5,000–₱35,000 per Circular 2024-006 (DEBT-030); NCR entry replaced by Wage Order NCR-28 (₱755/day non-agri, effectiveFrom 2026-09-26, dispute over NCR-27 documented in the source note; operational floor NCR-26 ₱695 until that date) (DEBT-031). Both `statutorySources` entries now `official` with evidence files updated. Pack re-signed (dual Ed25519, keyIds 2ebf509a…/fc56acd8…); goldens re-baselined (NCR monthly floor ₱16,610). **Still open (unchanged):** B2b external legal opinion and B1 pilot portal validation keep `commercialReady: false`.
...
## DEBT-027 — Non-blocking lint warnings (Etapa 0, 2026-09-01)

CI does not use `--max-warnings 0`; the 11 warnings below are tolerated and non-fatal:

- 9× `react-refresh/only-export-components` in shadcn UI primitives (`badge`, `button`, `form`,
  `navigation-menu`, `sidebar`, `toggle`), `CountryPackCard.tsx`, `companyContext.tsx`, `i18n.tsx`.
- 2× `react-hooks/exhaustive-deps`: `companyContext.tsx:32` (`companies` memoization),
  `_authenticated/calendar.tsx:88` (missing `refresh` dep).

Fix by extracting non-component exports into separate modules and memoizing the flagged expressions.
Not a release blocker.

## Etapa 0 note — `previewAuthStorage.ts` excluded from lint

`src/integrations/supabase/previewAuthStorage.ts` is auto-generated by the platform integration
