...
11. **Testes e conformidade** — `src/packs/indonesia/__tests__/separation.test.ts`: vetores por motivo, casos fracionários propositais (11 meses; 2 anos e 11 meses; 8 anos e 3 meses), pedido de demissão (só UPH + uang pisah), diarista e trabalhador por produção, `fixedAllowances` ausente devolvendo `complete: false`, PKWT acima do limite gerando violação sem conversão, desligamento em/após 31/10/2026 devolvendo `BLOCKED_PENDING_REGULATORY_REVALIDATION`, e desligamento anterior a 31/10/2024 devolvendo `BLOCKED_MISSING_HISTORICAL_RULESET`. Gate local `bunx tsgo --noEmit`, `bun test`, `bunx eslint .` verdes.
...
- Migração `separation_cases`: CREATE TABLE → GRANT authenticated/service_role → ENABLE RLS → trigger updated_at. Forward-only, aplicação deliberada. **RLS endurecida:** o escopo por dono da empresa (`owns_company`) é preservado, mas não basta — a tabela guarda salário, valores e aprovadores. As policies exigem também permissão funcional específica (`SEPARATION_VIEW`, `SEPARATION_CALCULATE`, `SEPARATION_APPROVE`, `SEPARATION_FINALIZE`) via `has_capability`, e a escrita financeira acontece **somente** por server function/RPC (browser → autorização → cálculo → persistência), nunca por INSERT/UPDATE arbitrário do frontend. Isso viabiliza a segregação: analista calcula, gerente revisa, aprovador autorizado finaliza — coerente com `approvals[]`/`approved_by`/`approved_at` já previstos.
