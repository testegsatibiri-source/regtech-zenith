...
1. **Hero** — global positioning, enterprise tone. CTAs: "Explore Country Packs" and "Talk to us". Keep the compliance-score card, jurisdiction-neutral signals.
...
5. **Trust & architecture** — signed and versioned packs, audit trails, runtime health gates, isolation between jurisdictions. The proof that the platform is engineered, not assembled.
6. **For developers** — API / SDK / signed packs, linking to `/api-docs`.
7. **Enterprise contact** — a closing band: "Deploying payroll compliance across multiple jurisdictions? Talk to our team." No prices, no plan cards, no per-employee figures.
8. **Footer** — three columns: Product, Country Packs (generated from the catalog), Company/Legal.

No pricing anywhere on the landing page. The whole Pricing section, the three plan cards and the add-on footnote are removed, and no `/pricing` route is created — commercial terms are handled in conversation, which is what enterprise buyers expect. The `#pricing` nav link is dropped from the header.
...
- Every global route gets its own `head()` (title, description, og:title, og:description); `/packs/indonesia/calculator` is new and needs its own.
- Header CTA labels stop coming from the i18n hero dictionary and use fixed English strings on global pages.
- Sitemap gains the per-pack calculator routes; `/calculator` is excluded once it redirects.

## Technical notes

Files touched: `src/routes/index.tsx` (loader source, restructure, pricing removal), new `src/routes/packs.$country.calculator.tsx`, `src/routes/calculator.tsx` (becomes a redirect), `src/components/SiteHeader.tsx` (nav, no pricing link, no lang toggle), a new footer component, `src/routes/sitemap[.]xml.ts`, and a new landing invariant test. No changes to the SDK, the country packs, backend, RLS or the authenticated app.

## Out of scope

No new jurisdictions, no design-system change, no per-country domains, no pricing page.
