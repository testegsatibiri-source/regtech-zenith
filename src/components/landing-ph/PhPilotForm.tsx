    ...
    <section id="form" className="px-4 py-16">
      <div className="mx-auto max-w-2xl">
        <div className="mb-10 text-center">
          <h2 className="mb-4 text-3xl font-semibold tracking-tight">{t("ph.cta.title")}</h2>
          <p className="text-muted-foreground">{t("ph.cta.body")}</p>
        </div>
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">{t("ph.form.submit")}</CardTitle>
          </CardHeader>
          <CardContent>
            {status === "success" ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-2">
                  <Label htmlFor="ph-email">{t("ph.form.email")}</Label>
                  <Input
                    id="ph-email"
                    type="email"
                    value={form.email}
                ...
                <div className="space-y-2">
                  <Label htmlFor="ph-companyName">{t("ph.form.companyName")}</Label>
                  <Input
                    id="ph-companyName"
                ...
                <div className="space-y-2">
                  <Label>{t("ph.form.employeeRange")}</Label>
                  <Select
                    value={form.employeeRange}
                    onValueChange={(v) => setForm((s) => ({ ...s, employeeRange: v }))}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={t("ph.form.employeeRangePlaceholder")} />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  <Label>{t("ph.form.role")}</Label>
                  <Select
                    value={form.role}
                    onValueChange={(v) => setForm((s) => ({ ...s, role: v }))}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={t("ph.form.rolePlaceholder")} />
                    </SelectTrigger>
                    <SelectContent>
                      {roles.map((r) => (
                        <SelectItem key={r} value={r}>
                ...
                <div className="flex items-start gap-3">
                  <Checkbox
                    ...
                    required
                  />
                  <Label htmlFor="ph-consent" className="text-sm font-normal leading-relaxed">
                    {t("ph.form.consent")}{" "}
                    <a href="/ph/patakaran-sa-privacy" className="text-primary underline">
                      {t("ph.form.consentLink")}
                    </a>
                    .
                  </Label>
                </div>
                {status === "error" && (
                  <p className="text-sm text-destructive">{t("ph.form.error")}</p>
                )}
                <Button
                  type="submit"
                  className="w-full"
