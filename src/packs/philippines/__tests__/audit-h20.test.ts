...
describe("PH tax bracket continuity", () => {
  it("has no gap between published brackets", () => {
    ...
    for (const gross of [20_833, 20_833.5, 33_332.5, 66_666.5, 166_666.5, 666_666.5]) {
      expect(() =>
        calculatePhTax({ monthlyGross: gross, maritalStatus: "single", hasNpwp: true }),
      ).not.toThrow();
    }
  });
  ...
  it("exempts income at or below the zero bracket", () => {
    expect(
      calculatePhTax({ monthlyGross: 20_833, maritalStatus: "single", hasNpwp: true }).tax,
    ).toBe(0);
    expect(
      calculatePhTax({ monthlyGross: 21_000, maritalStatus: "single", hasNpwp: true }).tax,
    ).toBeGreaterThan(0);
  });
});
...
describe("PH pack manifest & health", () => {
  ...
  it("health covers every declared engine capability", async () => {
    ...
    for (const check of [
      "tax.calculate.smoke",
      "benefits.calculate.smoke",
      "thirteenth.calculate.smoke",
      "contracts.validate.smoke",
    ]) {
