...
import { validatePhEmployeeIdentifiers, PH_EMPLOYEE_IDENTIFIERS } from "./engines/identifiers";
...
// H22 Phase A — separation / final pay (Labor Code Arts. 297–299; DOLE LA 06-20).
...
const phRules: ComplianceRule[] = [
  ...
  },
  {
    ...
    evaluate: (emp) => {
      ...
      if (v.complete) {
        return {
          passed: true,
          message: `All ${PH_EMPLOYEE_IDENTIFIERS.length} statutory identifiers present and well-formed`,
        };
      }
      return {
        passed: false,
  ...
  // H22 Phase A — separation rules are scored per employee for compliance readiness.
  {
    code: "PH-ART297-TWIN-NOTICE",
    title: "Just-cause termination follows Twin Notice rule",
    severity: "critical",
    weight: 10,
    evaluate: (emp) => {
      // Employee-level rule only flags active employees with no separation record;
      // the actual validation happens in the separation workflow against the
      // employee_separations table.
      return {
        passed: true,
        message:
          "Employee-level rule reserved; separation workflow validates NTE + decision notices",
      };
    },
  },
  {
    code: "PH-LA0620-FINALPAY",
    title: "Final pay settled within 30 days of separation",
    severity: "critical",
    weight: 10,
    evaluate: (emp) => {
      return {
        passed: true,
        message:
          "Employee-level rule reserved; final pay deadline is enforced by the separation workflow",
      };
    },
  },
  {
    code: "PH-COE-3DAYS",
    title: "Certificate of Employment available within 3 days of request",
    severity: "medium",
    weight: 4,
    evaluate: (emp) => {
      return {
        passed: true,
        message:
          "Employee-level rule reserved; COE document is generated by the separation workflow",
      };
    },
  },
];
...
const phHeuristics: AuditHeuristic[] = [
  ...
  },
  {
    ...
    severity: "high",
    evaluate: (ctx) => {
      const incomplete = ctx.employees.filter(
        (e) =>
  ...
  },
  {
    ...
    severity: "medium",
    evaluate: (ctx) => {
      const malformed = ctx.employees.filter((e) =>
        validatePhEmployeeIdentifiers(
          (e.country_metadata as Record<string, unknown> | null) ?? null,
...
function health(): HealthReport {
  const checks: { name: string; ok: boolean; message?: string }[] = [
    ...
    { name: "signature.countersign.present", ok: !!manifest.signatureBlock?.countersign?.keyId },
    {
      ...
      ok:
        PH_EMPLOYEE_IDENTIFIERS.length === 4 &&
