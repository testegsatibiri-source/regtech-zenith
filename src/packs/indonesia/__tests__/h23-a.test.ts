...
describe("BPJS 2026 parameters", () => {
  ...
  it("caps health and JP contributions", () => {
    ...
    expect(result.employee.health).toBe(Math.round(BPJS_2026.health.cap * BPJS_2026.health.employeeRate));
  ...
  it("optionally includes JKP financed by government + recomposition", () => {
    ...
    expect(result.jkp!.total).toBe(result.jkp!.government + result.jkp!.jkkRecomposition + result.jkp!.jkmRecomposition);
...
describe("Overtime (Lembur)", () => {
  ...
  it("applies 2x then 3x on public holidays", () => {
    const result = calculateOvertime({ monthlySalary: 5_200_000, hours: 10, dayType: "public-holiday", pattern: "5x8" });
    const base = 5_200_000 / 173;
  ...
  it("uses 7 regular hours for 6x7 pattern on rest days", () => {
    const result = calculateOvertime({ monthlySalary: 5_200_000, hours: 8, dayType: "rest-day", pattern: "6x7" });
    const base = 5_200_000 / 173;
...
describe("Annual PPh 21 reconciliation", () => {
  it("reports underpaid when annual liability exceeds withheld TER", () => {
    const result = reconcileAnnualPph21({ year: 2026, annualGross: 200_000_000, withheldTerTotal: 10_000_000 });
  ...
  it("reports over-withheld when TER exceeds annual liability", () => {
    const result = reconcileAnnualPph21({ year: 2026, annualGross: 80_000_000, withheldTerTotal: 5_000_000 });
