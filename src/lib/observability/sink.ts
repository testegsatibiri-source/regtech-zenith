...
class MultiSink implements MetricSink {
  ...
  async ingest(events: MetricEvent[]): Promise<void> {
    await Promise.all(this.sinks.map((s) => s.ingest(events).catch((e) => {
      // never let a sink failure break the caller
      console.warn(`[metrics:${s.name}] ingest failed`, e);
    })));
  }
  add(sink: MetricSink) { this.sinks.push(sink); }
  list(): readonly MetricSink[] { return this.sinks; }
}

class NoopSink implements MetricSink {
  name = "noop";
  async ingest(): Promise<void> { /* discard */ }
}

const registry = new MultiSink([new NoopSink()]);

export function registerSink(sink: MetricSink): void { registry.add(sink); }
export function getSinks(): readonly MetricSink[] { return registry.list(); }
