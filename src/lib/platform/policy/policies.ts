...
const adminOnly = (label: string) => (ctx: PolicyContext): Decision =>
...
const adminOrOperator = adminOr(
  (ctx) => hasRole(ctx, "platform_operator"),
  "platform_operator",
);
...
export const POLICIES: Record<PlatformAction, Policy> = {
  ...
  "pack.uninstall":       { action: "pack.uninstall",       decide: adminOnly("only platform_admin can uninstall packs") },
  ...
  "release.publish":      { action: "release.publish",      decide: adminOnly("only platform_admin can publish releases") },
  "release.rollback":     { action: "release.rollback",     decide: adminOnly("only platform_admin can roll back releases") },
  ...
  "parameters.edit":      { action: "parameters.edit",      decide: () => DENY("parameters editing disabled until DEBT-022 lands") },
  ...
  "audit.view":           { action: "audit.view",           decide: (ctx) =>
