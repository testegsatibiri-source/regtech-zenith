...
export const Route = createFileRoute("/packs/$country")({
  ...
  head: ({ loaderData }) => {
    ...
    if (!p) {
      return {
        meta: [{ title: "Pack unavailable | UBoard Asia" }, { name: "robots", content: "noindex" }],
      };
    }
      ...
      <div className="min-h-screen">
        ...
        <main className="mx-auto max-w-4xl px-4 py-16">
          ...
          <header className="mt-6 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                pack v{pack.version} · ruleset {pack.rulesetVersion} · interface{" "}
                {pack.interfaceVersion} · {pack.currency}
              </p>
          ...
          <div className="mt-10 flex gap-3">
            <Button asChild>
              <Link to="/auth">Start with {pack.name}</Link>
            </Button>
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-2xl px-4 py-24 text-center">
        ...
        <Button asChild className="mt-6">
          <Link to="/packs">Browse available packs</Link>
        </Button>
      </main>
    </div>
  );
