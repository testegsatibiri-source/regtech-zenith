  ...
  head: () => ({
    meta: [
      { title: "Global payroll compliance infrastructure | UBoard Asia" },
      {
        name: "description",
        content:
          "One global compliance core, independent signed country packs — payroll, tax and statutory compliance for enterprises operating across Southeast Asia.",
      },
      {
        property: "og:title",
        content: "Global compliance infrastructure with independent country packs",
      },
      {
        property: "og:description",
        content:
          "One secure core, independent signed country packs. Legislative change becomes a config update, not a code migration.",
      },
    ...
    <div className="min-h-screen">
        ...
        <div className="mx-auto max-w-6xl px-4">
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            <Step
              ...
              icon={Boxes}
              title="Global Core"
              desc="Companies, employees, payroll orchestration, audit trail and API — identical in every market."
            />
            <Step
              n="02"
              icon={Globe2}
              title="Country Pack"
              desc="A signed, versioned module carrying the tax, benefits and labour rules of one jurisdiction."
            />
            <Step
              n="03"
              icon={FileCheck2}
              title="Compliance evidence"
              desc="Every calculation is traced to the ruleset version that produced it, ready for inspection."
            />
          </div>
        </div>
      </section>
      ...
      <section id="platform" className="mx-auto max-w-6xl px-4 py-20">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-3">
          <Feature
            ...
            desc="Central compliance infrastructure for global payroll operations."
          />
          <Feature
            icon={Globe2}
            title="Employee lifecycle"
            desc="Manage employee events from onboarding to offboarding."
          />
          <Feature
            icon={Cpu}
            title="Payroll calculations"
            desc="Modular payroll calculation engines powered by country packs."
          />
          <Feature
            icon={Activity}
            title="Audit trails"
            desc="Secure records and compliance evidence for every transaction."
          />
          <Feature
            icon={Zap}
            title="API integrations"
            desc="Connect payroll workflows with enterprise systems."
          />
          <Feature
            icon={Bot}
      ...
      <section id="coverage" className="border-t border-border bg-muted/30 py-20">
        <div className="mx-auto max-w-6xl px-4">
          ...
          <Button asChild variant="outline" className="mt-10">
            <Link to="/packs">
              Explore all country packs <ArrowRight className="ml-1 h-4 w-4" />
            </Link>
          </Button>
        </div>
      </section>
      ...
      <section id="architecture" className="mx-auto max-w-6xl px-4 py-20">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2">
          <Feature
            icon={Lock}
            title="Cryptographically signed packs"
            desc="Ed25519 signatures verified against a trust store before a pack is allowed to run."
          />
          <Feature
            icon={GitBranch}
            title="Versioned rulesets"
            desc="Pack version, ruleset version and interface version are recorded on every calculation."
          />
          <Feature
            icon={HeartPulse}
            title="Runtime health gates"
            desc="A degraded pack drops out of production visibility on the next request — no deploy required."
          />
          <Feature
            icon={Layers}
            title="Jurisdictional isolation"
            desc="Packs never call each other and never patch the core, so one market cannot destabilise another."
          />
        </div>
      </section>
        ...
        <div className="mx-auto max-w-6xl px-4">
          ...
          <div className="mt-10 flex justify-center gap-3">
            ...
            <Button asChild variant="ghost">
              <Link to="/packs">See pack contracts</Link>
            </Button>
          </div>
        </div>
      </section>
      ...
      <section id="contact" className="mx-auto max-w-4xl px-4 py-24 text-center">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          ...
          <Button asChild size="lg" variant="outline">
            <Link to="/packs">Review coverage</Link>
          </Button>
        </div>
      </section>
...
function Signal({ text, ok }: { text: string; ok?: boolean }) {
  return (
    <div className="flex items-center gap-2 rounded-md bg-white/5 px-3 py-1.5">
      <span
        className={
          "h-2 w-2 rounded-full " + (ok ? "bg-[oklch(0.7_0.15_155)]" : "bg-[oklch(0.78_0.16_75)]")
        }
      />
