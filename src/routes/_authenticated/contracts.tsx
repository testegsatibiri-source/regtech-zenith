...
function Contracts() {
  ...
  async function save() {
    if (!draft) return;
    if (draft.contract_type === "PKWT" && !draft.end_date)
      return toast.error("PKWT requires an end date");
    try {
  ...
  const stats = {
    ...
    coverage: emps.length
      ? Math.round(
          (cs.filter((c) => c.status === "active" && c.employee_id).length / emps.length) * 100,
        )
      : 0,
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground">
            PKWT / PKWTT · PP 35/2021 · UU 13/2003 (Omnibus Law)
          </p>
        </div>
        <Button onClick={() => edit()}>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:col-span-2">
          ...
          <Stat
            ...
            tone={stats.coverage < 100 ? "warn" : "ok"}
          />
          <Stat
            icon={AlertTriangle}
            label="Expiring ≤ 60d"
            value={String(stats.expiring)}
            tone={stats.expiring ? "warn" : "ok"}
          />
          <Stat
            icon={AlertTriangle}
            label="Expired"
            value={String(stats.expired)}
            tone={stats.expired ? "warn" : "ok"}
          />
        </div>
      </div>
                ...
                return (
                  <TableRow key={c.id}>
                    <TableCell className="font-medium">
                      {emp?.full_name ?? <span className="text-muted-foreground">Unassigned</span>}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right">
                      ...
                      <Button size="icon" variant="ghost" onClick={() => c.id && remove(c.id)}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{draft?.id ? "Edit" : "New"} contract</DialogTitle>
          </DialogHeader>
          {draft && (
            <div className="grid gap-3 sm:grid-cols-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => {
                    ...
                    setDraft({
                      ...
                      base_salary:
                        emp && draft.base_salary === 0 ? emp.base_salary : draft.base_salary,
              ...
              <F label="Contract type">
                <Select
                  ...
                  onValueChange={(v) =>
                    setDraft({ ...draft, contract_type: v as "PKWT" | "PKWTT" })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <F label="Status">
                <Select
                  value={draft.status}
                  onValueChange={(v) => setDraft({ ...draft, status: v as ContractLike["status"] })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <F
                label={
                  draft.contract_type === "PKWT" ? "End date (required)" : "End date (optional)"
                }
              >
                <Input
                  type="date"
              ...
              <F label="Probation ends" className="sm:col-span-2">
                <Input
                  ...
                  onChange={(e) =>
                    setDraft({ ...draft, probation_end_date: e.target.value || null })
                  }
                />
                {draft.contract_type === "PKWT" && draft.probation_end_date && (
                  <p className="text-xs text-destructive">
                    PKWT cannot include probation (UU 13/2003 art. 58).
                  </p>
                )}
              </F>
