...
function blank(companyId: string): Emp {
  return {
    company_id: companyId, full_name: "", position: "", department: "",
    base_salary: 5000000, marital_status: "TK/0", religion: "Islam",
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">{employees.length} people · identifiers drive the Compliance Score</p>
        </div>
        <Button onClick={() => edit()}><Plus className="mr-1 h-4 w-4" /> Add employee</Button>
      </div>
                ...
                return (
                  <TableRow key={e.id}>
                    ...
                    <TableCell className="text-right">
                      ...
                      <Button size="icon" variant="ghost" onClick={() => e.id && remove(e.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                    </TableCell>
                  </TableRow>
                );
              })}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          <DialogHeader><DialogTitle>{draft?.id ? "Edit" : "Add"} employee</DialogTitle></DialogHeader>
          {draft && (
