...
function CalendarPage() {
  ...
  async function handleSeed() {
    ...
    try {
      ...
      toast.success(
        `Seeded ${r.inserted} obligation(s) for ${year}${r.skipped ? ` (${r.skipped} already present)` : ""}`,
      );
      await refresh();
    } catch (e) {
    ...
    return (
      <div className="mx-auto max-w-md py-20 text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          Pick a company in the top bar to view its regulatory calendar.
        </p>
      </div>
    );
  }
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="py-10 text-center text-sm text-muted-foreground">
              No obligations yet. Click <strong>Seed {year} obligations</strong> to bootstrap the{" "}
              {activePack.name} calendar.
            </div>
          ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-0.5 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                        ...
                        {r.base_legal && (
                          <>
                            <span>·</span>
                            <span className="truncate">{r.base_legal}</span>
                          </>
                        )}
                      </div>
                    </div>
