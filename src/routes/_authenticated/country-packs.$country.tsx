  ...
  head: ({ params }) => ({
    meta: [
      { title: `${params.country.toUpperCase()} Country Pack · UBoard Asia` },
      {
        name: "description",
        content:
          "Manifest, engines, events, validator report and live health for this installed country pack.",
      },
    ],
  }),
  ...
  return (
    <div className="mx-auto max-w-3xl p-6">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        This country pack is not present in the runtime.
      </p>
      <Button asChild className="mt-4">
        <Link to="/country-packs">Back to packs</Link>
      </Button>
    </div>
  );
}
...
function CountryPackDetailPage() {
  ...
  const [health, setHealth] = useState<
    Record<string, HealthReport | { status: "loading" | "error"; checks: never[] }>
  >({});
      ...
      for (const rec of installed) {
        ...
        try {
          ...
          if (!cancelled)
            setHealth((s) => ({
              ...s,
              [rec.pack.manifest.country]: { status: "error", checks: [] },
            }));
        }
      }
    })();
            ...
            <Card key={m.country}>
              ...
              <CardContent className="space-y-4">
                {reason && (
                  <p
                    className={
                      "text-xs " + (status === "degraded" ? "text-warning" : "text-destructive")
                    }
                  >
                    {reason}
                  </p>
                ...
                {m.events?.emits?.length || m.events?.consumes?.length ? (
                  <Section title="Events">
                    {m.events!.emits?.map((e) => (
                      <Badge key={"e" + e} className="border-0 bg-primary/10 text-primary">
                        ↑ {e}
                      </Badge>
                    ))}
                    {m.events!.consumes?.map((e) => (
                      <Badge key={"c" + e} className="border-0 bg-accent/10 text-accent">
                        ↓ {e}
                      </Badge>
                    ))}
                  </Section>
